...
Form(bool sign, std::string name, int sign_grade, int execute_grade):_sign(false), _sign_grade(sign_grade), _execute_grade(execute_grade), _name(name)
{
...
std::ostream        &operator<< (std::ostream &out, const Form &rhs) {
    out << "The form: " << rhs.getName() << " which is " << (rhs.getSign() ? "" : "not ") << " signed, which requires a grade " << rhs.getSignGrade() << " to sign, and " << rhs.getExecuteGrade() << " to be executed." << std::endl;
    return (out);
}
